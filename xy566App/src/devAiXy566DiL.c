...
static long xy566DiL_init_record();
static long xy566DiL_get_ioint_info();
static long xy566DiL_read_ai();
static long xy566DiL_special_linconv();
...
struct {
   ...
   NULL,
   NULL,
   xy566DiL_init_record,
   xy566DiL_get_ioint_info,
   xy566DiL_read_ai,
   xy566DiL_special_linconv};
epicsExportAddress(dset,devAiXy566DiL);


static long xy566DiL_init_record(struct aiRecord *pai)
{
   ...
   if((status=ai_xy566_driver(pvmeio->card,pvmeio->signal,XY566DIL,&value))) {
      recGblRecordError(status,(void *)pai, "devAiXy566DiL (init_record) ai_xy566_driver error");
      return(status);
   }
   return(0);
}

static long xy566DiL_get_ioint_info(int cmd, struct aiRecord *pai,IOSCANPVT *ppvt)
{
   ai_xy566_getioscanpvt(pai->inp.value.vmeio.card,ppvt);
   return(0);
}

static long xy566DiL_read_ai(struct aiRecord *pai)
{
   unsigned short value;
   struct vmeio *pvmeio;
   long status;
...
static long xy566DiL_special_linconv(struct aiRecord *pai, int after)
{
